...
class CoinListViewModel(private val repository: CoinRepository) :
    ...
    private fun getCoins() {
        viewModelScope.launch {
            ...
            if (firstLoading) {
               ...
               _uiState.value = _uiState.value.copy(data = data, isLoading = false, error = null)
            }
            delay(2000)
    ...
    private fun handleCoinsResult(coinResult: Result<List<Coin>>) {
        ...
        coinResult.fold(onSuccess = { coins ->
            ...
            val isPageLoading =  if (_uiState.value.isLoading == true) false else _uiState.value.isLoading
            val pageError =  if (_uiState.value.isLoading == true) it else _uiState.value.error
            val newData = data?.copy(
                isLoading = false,
                error = pageError ?: it)

            _uiState.value = _uiState.value.copy(data = newData, isLoading =  isPageLoading, error =  pageError)
        })
    }
