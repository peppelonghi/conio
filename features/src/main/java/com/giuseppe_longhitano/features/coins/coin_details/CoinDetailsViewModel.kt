    ...
    override fun handleEvent(uiEvent: UIEvent) {
        when (uiEvent) {
            ...
            is CoinDetailsEvent.RefreshGraph -> getChartData(
                hourInterval = HourInterval.safeValueOf(_uiState.value.data?.chart?.data?.hourInterval),
                dayInterval = DayInterval.safeValueOf(_uiState.value.data?.chart?.data?.dayInterval)
            )

            is CoinDetailsEvent.OnIntervalChange ->
                getChartData(
                    hourInterval = uiEvent.hourInterval,
                    dayInterval = DayInterval.safeValueOf(
            ...
            is CoinDetailsEvent.OnDaysChange ->
                getChartData(
                    ...
                    hourInterval = HourInterval.safeValueOf(
